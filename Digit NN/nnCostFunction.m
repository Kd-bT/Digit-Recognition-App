...
for i = 1:m
    ...
    cost_i   = sum((-y_i .* log(h_i)) - ((ones(num_labels, 1) - y_i) .* log((ones(num_labels, 1) - h_i))));
    costSum  = costSum + cost_i;
end

J = (1 / m) * costSum;

relTheta1 = Theta1(:, 2:end);
relTheta2 = Theta2(:, 2:end);
regTerm = (lambda / (2 * m)) * (sum(relTheta1(:).^2) + sum(relTheta2(:).^2));
